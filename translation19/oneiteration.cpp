...
void OneIteration(char* str, long length, int ** matrix_forward, int ** matrix_reverse, 
                  ...
                  struct LastReportedRepeat right, long int offset, bool isend, FILE* outfile)
{

   /***************************** Variable Initialization *********************************/

   char *mid_forward = NULL, *mid_backward = NULL, *right_forward = NULL, *right_backward = NULL, 
        *left_forward = NULL, *left_backward = NULL, *rev_pattern = NULL, *rev_text = NULL, 
        *mid_text = NULL, *mid_pattern = NULL;
   int i, j, k,
       begpos, endpos,
       n = length/2,
       ...
       rev_pattern_len = 0, rev_text_len = 0, 
       mid_text_len = 0, mid_pattern_len = 0,
       **temp = NULL,
       tempNum,
       k1,
   ...
   for (j = MIN_PERIOD; j <= n - 1 + length%2; j++)
   {
    for (direction = 0 ; direction < 2 ; direction++)
    {
        ...
        for (i = 0; i <= MAX_ERRORS; i++)
        {
            ...
            // Fill errors arrays:
            for (k = MAX_ERRORS - i; k <= MAX_ERRORS + i; k++)
      	    {
        ...
        if ( prev->newhigh == 1 && comparison == -1 )
        {
                                     ...
                                     ||prev->right >= left_right + SHIFT)) )
            {
              ...
              {
                    tracealign (prev->matrix_reverse, rev_pattern-diff, rev_text,
                                prev->down_errors[prev->left_errs].row-1,
                                prev->down_errors[prev->left_errs].col-1,
                                pos1, pos2, state, outfile);
                    traceforward (prev->matrix_forward, mid_pattern-diff, mid_text,
                                  prev->right_errors[prev->right_errs].row-1,
                                  prev->right_errors[prev->right_errs].col-1,
                                  pos1, pos2, state, outfile, length);
              }
              else
              {
                    tracealign (prev->matrix_reverse, rev_pattern, rev_text+diff, 
                                prev->down_errors[prev->left_errs].row-1,
                                prev->down_errors[prev->left_errs].col-1,
                                pos1, pos2, state, outfile);
                    traceforward (prev->matrix_forward, mid_pattern, mid_text+diff,
                                  prev->right_errors[prev->right_errs].row-1,
                                  prev->right_errors[prev->right_errs].col-1,
                                  pos1, pos2, state, outfile, length);
              }
