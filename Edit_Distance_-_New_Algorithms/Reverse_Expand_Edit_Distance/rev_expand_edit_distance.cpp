...
bool testingEditDistance (int **, char const *, int, char const *, int, int);
...
bool algo (char const *argv[])
{
   ...
   for (k = 0; k < pure_half; k++)
   {
       ...
       if (!testingEditDistance (matrix, argv[1] - whatRow, text_len - whatRow, 
                                         argv[1] - pure_half, text_len - pure_half, whatRow))
       {
          for (i = 0; i < text_len + 1; i++)
             free (matrix[i]);
          free (matrix);
