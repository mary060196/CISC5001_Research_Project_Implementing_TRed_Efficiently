...
// in fact the last 2 on the -2 diagonal, which is what the KxK matrix states.
...
void buildmatrixforward (int **m, int *upper, int *lower, const char *s1, const int len_s1, 
                         const char *s2, const int len_s2, const int limit)
{
   int reps = len_s1 + 1,     // Number of rows in the NxN matrix
...
/********* CASE IF ALLOWED ERRORS > STRING LENGTH OR LIMIT IS REACHED**********/
   ...
   // No need anymore to initialize elements to -1s!
   // The program was changed to skip over the -1s regions, thus
   //    considerably increasing the speed of the program.
...
// m is a pointer to the KxK matrix
void buildmatrixbackward (int **m, int *upper, int *lower, const char *s1, const int len_s1, 
                          const char *s2, const int len_s2, const int limit)
{
   int reps = len_s1 + 1,     // Number of rows in the NxN matrix
